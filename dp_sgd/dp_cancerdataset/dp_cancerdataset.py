...
NUM_TESTING_IMAGES = 123
NUM_VALIDATION_IMAGES = 15 # totol 150
INPUT_SIZE = 9
LABEL_SIZE = 2
...
tf.flags.DEFINE_string("training_data_path",
                       "./differential_privacy/datasets/cancer/cancer_train.csv.tfrecords",
                       "Location of the training data.")
tf.flags.DEFINE_string("eval_data_path",
                       "./differential_privacy/datasets/cancer/cancer_test.csv.tfrecords",
...
def get_current_sigma(epoch):
    # Time-based decay
    # curr_sigma= adp.inverse_time_decay(10.0, epoch, 0.0132)
    # constant
    # curr_sigma = 25.0
    # step decay
    # curr_sigma=adp.step_decay(30.0, epoch, 0.5782, decay_steps=200)
    # expoential decay
    curr_sigma = adp.exponential_decay(30.0, epoch, 0.001)
    # polynomial
    # curr_sigma=adp.polynomial_decay(10.0, epoch, decay_steps=100, end_sigma=2, power=1.432)

    # constant-then-decay
    # curr_sigma=adp.consthenexp_decay(25.0, epoch, 0.002, 300)
    return curr_sigma

def Train(train_file, test_file, network_parameters, num_steps,
    ...
    for step in range(num_steps):
      ...
      if with_privacy:
        ...
        if epoch-previous_epoch==1:
          rho_tracking.append(rho_tracking[-1]+1.0/(2.0*curr_sigma**2))

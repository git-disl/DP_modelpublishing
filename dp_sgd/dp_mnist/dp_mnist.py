...
tf.flags.DEFINE_integer("hidden_layer_num_units", 200,
...
# generate sigma from a decay schedule
def get_current_sigma(epoch):
    # Time-based decay
    # curr_sigma = adp.inverse_time_decay(10.0, epoch, 0.0281)
    # step decay
    # curr_sigma = adp.step_decay(10.0, epoch, 0.7008, decay_steps=10)
    # expoential decay
    curr_sigma = adp.exponential_decay(10.0, epoch, 0.0138)
    # polynomial
    # curr_sigma = adp.polynomial_decay(10.0, epoch, decay_steps=100, end_sigma=2, power=3.5277)
    return curr_sigma
...
def Train(mnist_train_file, mnist_test_file, network_parameters, num_steps,
  ...
  print(FLAGS.hidden_layer_num_units)
  ...
  with tf.device('/gpu:0'), tf.Graph().as_default(), tf.Session() as sess:
    ...
    for step in range(num_steps):
      ...
      #curr_eps not used because we use noise scale decided by privacy budget allocator
      curr_eps = utils.VaryRate(FLAGS.eps, FLAGS.end_eps,
                                FLAGS.eps_saturate_epochs, epoch)
      old_sigma = curr_sigma

      # get current sigma from a decay schedule
      curr_sigma = get_current_sigma(epoch)

      if step==0:
          print("rhototal="+str(rhototal)+" "+"curr_sigma="+str(curr_sigma))

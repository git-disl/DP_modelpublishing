...
tf.flags.DEFINE_string("training_data_path",
                       "/home/lei/Dropbox/projects/dpdeeplearning/data/mnist_validation/mnist_train.tfrecord",
                       "Location of the training data.")
tf.flags.DEFINE_string("eval_data_path",
                       "/home/lei/Dropbox/projects/dpdeeplearning/data/mnist_validation/mnist_test.tfrecord",
                       "Location of the eval data.")
tf.flags.DEFINE_string("validation_data_path",
                        "/home/lei/Dropbox/projects/dpdeeplearning/data/mnist_validation/mnist_validation.tfrecord",
                        "Location of the validation data.")

tf.flags.DEFINE_integer("eval_steps", 92,
...
tf.flags.DEFINE_float("pca_sigma", 16.0,
...
tf.flags.DEFINE_string("save_path", "/home/lei/Dropbox/projects/dpdeeplearning/save/mnist_dir_validation",
                       "Directory for saving model outputs.")

FLAGS = tf.flags.FLAGS
NUM_TRAINING_IMAGES = 55000
NUM_VALIDATION = 5000
...
def Train(mnist_train_file, mnist_test_file, mnist_validation_file, network_parameters, num_steps,
  ...
  with tf.device('/gpu:0'), tf.Graph().as_default(), tf.Session() as sess:
    ...
    filename = "results"+datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')+".json"
    ...
    for step in range(num_steps):
      ...
      if (eval_steps > 0 and (step + 1) % eval_steps == 0) or should_terminate:
        ...
        validation_accuracy, mistakes = Eval(mnist_validation_file, network_parameters,
                                       num_testing_images=NUM_TESTING_IMAGES,
                                       randomize=False, load_path=save_path,
                                       save_mistakes=FLAGS.save_mistakes)

...
class GaussianMomentsAccountant(MomentsAccountant):
  ...
  def _differential_moments(self, sigma, s, t):
    ...
    exponents = tf.cast([j * (j + 1.0 - 2.0 * s) / (2.0 * sigma * sigma)
                             for j in range(t + 1)], dtype=tf.float64)
...
#zCDP Privacy Accountant  --not used yet
class zCDPAccountant(object):
  """Keep track of privacy spending of zCDP.
    It has two parts each for random shuffling and random sampling.
    Note: For the random sampling, our approach uses a heuristic bound alpha*q^2/sigma^2 which we verified in a wide
    settings of parameters, therefore it produces only approximate estimation on the privacy loss under random sampling.
    By experiment, we show it produces a looser estimation but close result to Moments accountant method.
    Details can be found in: https://arxiv.org/abs/1904.02200
  """
  def __init__(self, total_examples, mode):
    self.mode = mode
    self._total_examples = total_examples
    self._accumed_rho = tf.Variable(numpy.float64(0.0),
                                     trainable=False,
                                     name=("accumed_rho"))
    self.min_u_alpha = tf.constant(200)

  def accumulate_privacy_spending(self, eps_delta, sigma, num_examples):
    if self.mode == 'RS':
      q = tf.cast(num_examples, tf.float64) * 1.0 / self._total_examples
      self.u_alpha = sigma*sigma*tf.log(1.0/q/sigma)+1
      self.min_u_alpha = tf.minimum(self.u_alpha, self.min_u_alpha)
      self._accumed_rho += q**2/sigma**2

    elif self.mode == 'RF':
      """For random shuffling, eps_data has indicator to show if the next epoch starts"""
      next_epoch, _ = eps_delta
      self._accumed_rho = tf.cond(tf.greater(next_epoch, 0), lambda: tf.add(self._accumed_rho, 1.0/(2.0*sigma**2)), lambda: self._accumed_rho)
    else:
      raise Exception('No mode option {}'.format(self.mode))

  def get_privacy_spent(self, sess, target_eps=None, target_deltas=None):
    assert target_deltas
    eps_deltas = []
    for delta in target_deltas:
      rho_c = sess.run(self._accumed_rho)
      if self.mode == 'RS':
        min_ualpha = sess.run(self.min_u_alpha)
        th = 1 / math.exp(rho_c*(min_ualpha-1)**2)
        if delta < th:
          eps = rho_c * min_ualpha - math.log(delta)/(min_ualpha-1)
        else:
          eps = rho_c + 2 * math.sqrt(rho_c * (math.log(1.0 / delta)))
      else:
        #model 'RF'
        eps = rho_c + 2 * math.sqrt(rho_c *(math.log(1.0 / delta)))
...
# Dump Placeholder for zCDPAccountant. Real zCDP Privacy accounting happens in the training code.
class DumpzCDPAccountant(object):
  """Keep track of privacy spending of zCDP."""
